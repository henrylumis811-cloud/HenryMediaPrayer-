...
class QueueAdapter(
    ...
    override fun onBindViewHolder(holder: QueueViewHolder, position: Int) {
        ...
        holder.binding.btnInfo.setOnClickListener {
            val duration = item.mediaMetadata.extras?.getLong("duration_ms", 0L) ?: 0L
            val path = item.mediaMetadata.extras?.getString("data_path")
            val id = item.mediaId.toLongOrNull() ?: return@setOnClickListener
            TrackInfoDialog.show(
                ...
                Song(
                    ...
                    durationMs = duration,
                    uriString = item.localConfiguration?.uri?.toString() ?: return@setOnClickListener,
                    albumId = -1L,
                    dataPath = path
                )
            )
