...
class SignalFragment : Fragment() {
    ...
    private fun renderEqualizer(eq: com.henrylumis.mediaprayer.audio.EqualizerController) {
        ...
        val toggle = com.google.android.material.switchmaterial.SwitchMaterial(ctx).apply {
            ...
            contentDescription = "Enable sound controls"
        ...
        val intro = TextView(ctx).apply {
            text = "Simple controls — no confusing frequency sliders."
            setTextColor(resources.getColor(com.henrylumis.mediaprayer.R.color.text_secondary, null))
            textSize = 12f
            setPadding(0, 10, 0, 12)
        }
        binding.equalizerContainer.addView(intro)

        val presetTitle = TextView(ctx).apply {
            text = "Sound presets"
            setTextColor(resources.getColor(com.henrylumis.mediaprayer.R.color.text_primary, null))
            textSize = 13f
            setTypeface(null, android.graphics.Typeface.BOLD)
        }
        binding.equalizerContainer.addView(presetTitle)
        ...
        listOf("Balanced", "Bass Boost", "Pop", "Rock", "Vocal", "Electronic").forEach { name ->
            val chip = com.google.android.material.chip.Chip(ctx).apply {
                text = name
                isCheckable = false
                setOnClickListener {
                    eq.applyPreset(name)
                    refreshSimpleEqBars(eq)
                }
        ...
        val presetScroll = android.widget.HorizontalScrollView(ctx).apply {
            isHorizontalScrollBarEnabled = false
            addView(presets)
        }
        binding.equalizerContainer.addView(presetScroll)

        addSimpleEqControl(eq, "Bass Boost", "Adds warmth and punch to low frequencies", eq.bassBoost, com.henrylumis.mediaprayer.audio.SimpleControl.BASS)
        addSimpleEqControl(eq, "Vocal Clarity", "Brings voices and vocals forward", eq.vocalClarity, com.henrylumis.mediaprayer.audio.SimpleControl.VOCAL)
        addSimpleEqControl(eq, "Treble Boost", "Adds brightness and detail", eq.trebleBoost, com.henrylumis.mediaprayer.audio.SimpleControl.TREBLE)
    }

    private fun addSimpleEqControl(
        eq: com.henrylumis.mediaprayer.audio.EqualizerController,
        title: String,
        subtitle: String,
        initial: Int,
        kind: com.henrylumis.mediaprayer.audio.SimpleControl
    ) {
        val ctx = requireContext()
        ...
        val valueView = TextView(ctx).apply {
            text = "$initial%"
            textSize = 13f
            setTextColor(resources.getColor(com.henrylumis.mediaprayer.R.color.accent_cyan, null))
        }
        ...
        val seek = SeekBar(ctx).apply {
            max = 100
            progress = initial
            progressTintList = android.content.res.ColorStateList.valueOf(resources.getColor(com.henrylumis.mediaprayer.R.color.accent_cyan, null))
            thumbTintList = android.content.res.ColorStateList.valueOf(resources.getColor(com.henrylumis.mediaprayer.R.color.accent_cyan, null))
            setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                override fun onProgressChanged(sb: SeekBar?, progress: Int, fromUser: Boolean) {
                    if (fromUser) {
                        eq.setSimpleControl(kind, progress)
        ...
        box.addView(seek)
        binding.equalizerContainer.addView(box)
    }

    private fun refreshSimpleEqBars(eq: com.henrylumis.mediaprayer.audio.EqualizerController) {
        // Re-rendering keeps all three percentage labels synchronized after a preset.
        equalizerBuilt = false
        renderEqualizer(eq)
    }
