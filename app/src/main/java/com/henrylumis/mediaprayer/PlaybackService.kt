...
class PlaybackService : MediaSessionService() {
    ...
    override fun onCreate() {
        ...
        bridge.onPlaybackStateChanged = {
            // Media3 already owns the mediaPlayback foreground service and the
            // internal ExoPlayers use WAKE_MODE_LOCAL. This additional partial
            // wake lock is deliberately held only while audio is actually playing,
            // reducing device-specific sleep/CPU suspension that can interrupt
            // long background sessions. It does not attempt to prevent Android from
            // killing the process under memory or user/device battery policies.
    ...
    private fun updatePlaybackWakeLock() {
        try {
            ...
            // Wake-lock failures must never interfere with playback.
        }
    }

    /** Keep the playback service restartable after an unexpected process kill.
     * Media3 still controls the actual foreground lifecycle and explicit stopSelf()
     * calls are not restarted by Android. */
    ...
    private fun applyNormalizationForCurrentTrack(mediaItem: MediaItem?) {
        ...
        serviceScope.launch {
            ...
            withContext(Dispatchers.Main) {
                ...
                // while quiet masters are not additionally boosted by ReplayGain. The
                // LoudnessNormalizer keeps its small constant output lift separately,
                // so normal playback remains a little fuller at the same system volume.
                loudnessNormalizer?.reset()
            ...
            val item = MediaItem.Builder()
                ...
                .setMediaMetadata(
                    MediaMetadata.Builder()
                        ...
                        .setArtworkUri(android.net.Uri.parse("content://media/external/audio/media/${saved.mediaId}/albumart"))
                        .setExtras(extras)
                        .build()
                )
                .build()
    ...
    override fun onDestroy() {
        ...
        serviceScope.cancel()
        sleepTimer.cancel()
        try { if (playbackWakeLock.isHeld) playbackWakeLock.release() } catch (_: Exception) {}
        equalizer?.release()
